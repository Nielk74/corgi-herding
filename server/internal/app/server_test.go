...
func TestHTTPValidationCapacityAndHealth(t *testing.T) {
	...
	if resp.StatusCode != 200 || health.Status != "ok" || health.Version != "test-version" || health.Sessions != 1 || health.Protocol != 1 || health.LayoutVersion != 1 || !reflect.DeepEqual(health.LayoutVersions, []int{1, 2, 3, 4, 5, 6, 7, 8}) {
...
func TestLandscapeSelectionSharedAndRestored(t *testing.T) {
	for _, tc := range []struct{ name, body, want string }{
		...
		{"cloud", `{"name":"Ada","landscape":"cloud"}`, "cloud"},
	} {
...
func TestUnknownSavedLayoutsFailWithoutOverwriting(t *testing.T) {
	for _, tc := range []struct {
		...
		{"unknown_version", game.LandscapeAlpine, &game.Layout{Version: 5}},
		...
		{"missing_cloud", game.LandscapeCloud, nil},
	} {
		t.Run(tc.name, func(t *testing.T) {
			w := game.New("ABCDEF")
