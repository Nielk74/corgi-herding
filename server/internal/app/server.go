...
func newHerd(s savedHerd, log *slog.Logger) *herd {
	...
	for i := range s.World.Players {
		...
		if s.World.Layout.RockPass == nil && s.World.Layout.Ridge == nil && s.World.Layout.Shore == nil {
...
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	...
	respond(w, code, map[string]any{"status": status, "version": s.cfg.Version, "protocol": 1, "layout_version": 1, "layout_versions": []int{1, 2, 3, 4, 5}, "sessions": n})
}
...
func (s *Server) create(w http.ResponseWriter, r *http.Request) {
	...
	world := game.NewForLandscape(code, landscape)
