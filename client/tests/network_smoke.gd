...
func _run() -> void:
	...
	if landscape in ["larch", "orchard"]:
		...
		if not await _until(func() -> bool: return _own_position().distance_to(Vector2(-10, 0)) < 0.3):
			...
			return
	if landscape == "oasis":
		if game.network.advertised_layout_version != 8 or game.meadow.gate != null or game.meadow.bridge != null:
	...
	if landscape == "cloud":
		if game.network.advertised_layout_version != 8 or game.meadow.gate != null or game.meadow.bridge != null:
			_fail("Cloud negotiates current v8 capability while retaining its v4 world without a river or gate")
			return
